...
export default function Room() {
  const initialRooms = [
    ...
    },
    {
      ...
      status: "available",
      floor: "3rd",
      amenities: ["WiFi", "AC", "TV", "Minibar", "Safe", "Workspace"],
    },
    {
      id: 6,
      name: "Suite 302",
  ...
  const displayRooms = useMemo(() => {
    ...
    return internalRooms.map((room) => {
      ...
      const bookingForRoom = activeBookings.find(
        (booking) => booking.roomNo === room.name,
      );
      if (!bookingForRoom) {
      ...
      return {
        ...room,
        status:
          bookingForRoom.status === "checked-in" ? "checked-in" : "booked",
      };
    });
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedRoom) {
      ...
      setInternalRooms([
        ...internalRooms,
        { ...formData, id: Date.now(), status: "available" },
      ]);
    }
    closeForm();
  };
  ...
  const updateStatus = (id, newStatus) => {
    setInternalRooms(
      internalRooms.map((r) => (r.id === id ? { ...r, status: newStatus } : r)),
    );
  };
